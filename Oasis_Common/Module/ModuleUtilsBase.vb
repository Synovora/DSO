...
Public Module ModuleUtilsBase
    ...
    ''' <summary>
    ''' Clé symétrique utilisée par EncryptString / DecryptString, notamment pour
    ''' transmettre la chaîne de connexion au client lourd via /api/login.
    '''
    ''' Elle était auparavant codée en dur dans ce fichier. Le dépôt étant public,
    ''' cette valeur est désormais connue de tous : elle doit être remplacée par une
    ''' clé propre à chaque déploiement, renseignée dans la configuration.
    '''
    ''' Le serveur et TOUS les clients déployés doivent porter la même valeur. Voir
    ''' la section "Cryptographic key" du README avant tout déploiement.
    ''' </summary>
    Private ReadOnly Property CleDeChiffrement As String
        Get
            Dim cle = ConfigurationManager.AppSettings("OasisCryptoKey")
            If String.IsNullOrWhiteSpace(cle) Then
                Throw New ConfigurationErrorsException(
                    "Le paramètre 'OasisCryptoKey' est absent de la configuration. " &
                    "Renseignez-le dans Web.config (serveur) et App.config (client) " &
                    "avec la même valeur, d'au moins 32 caractères aléatoires. " &
    ...
    Public Function EncryptString(ByVal clearText As String) As String
        ...
        Dim CipherBytes As Byte() = Encrypt(plainText, CleDeChiffrement)
    ...
    Public Function DecryptString(ByVal cipherText As String) As String
        ...
        Dim plainTextData As Byte() = Decrypt(cipheredData, CleDeChiffrement)
