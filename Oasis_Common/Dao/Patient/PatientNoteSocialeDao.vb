...
Public Class PatientNoteSocialeDao
    Inherits StandardDao

    Public Function getAllNoteSocialebyPatient(patientId As Integer) As DataTable
        Dim SQLString As String = "select * from oasis.oa_patient_note_social where" &
        ...
        Using con As SqlConnection = GetConnection()
            Dim NoteSocialeDataAdapter As SqlDataAdapter = New SqlDataAdapter()
            Using NoteSocialeDataAdapter
                NoteSocialeDataAdapter.SelectCommand = New SqlCommand(SQLString, con)
                Dim NoteSocialeDataTable As DataTable = New DataTable()
                Using NoteSocialeDataTable
                    Try
                        NoteSocialeDataAdapter.Fill(NoteSocialeDataTable)
                        Dim command As SqlCommand = con.CreateCommand()
                    Catch ex As Exception
                        Throw ex
                    End Try
                    Return NoteSocialeDataTable
                End Using
            End Using
        End Using
    End Function

    Public Function getNoteById(noteId As Integer) As PatientNote
        ...
        Try
            ...
            command.CommandText =
                "select * from oasis.oa_patient_note_social where oa_patient_note_id = @id"
            command.Parameters.AddWithValue("@id", noteId)
            Using reader As SqlDataReader = command.ExecuteReader()
                If reader.Read() Then
                    patientNote = buildBean(reader)
                Else
                    Throw New ArgumentException("Note contexte social du patient inexistante !")
                End If
            End Using
    ...
    Public Shared Function buildBean(record As System.Data.IDataRecord) As PatientNote
        ...
        patientNote.NoteId = record("oa_patient_note_id")
        patientNote.PatientId = Coalesce(record("oa_patient_id"), 0)
        patientNote.PatientNote = Coalesce(record("oa_patient_note"), "")
        patientNote.UserCreation = Coalesce(record("oa_patient_note_utilisateur_creation"), 0)
        patientNote.DateCreation = Coalesce(record("oa_patient_note_date_creation"), Nothing)
        patientNote.UserModification = Coalesce(record("oa_patient_note_utilisateur_modification"), 0)
        patientNote.DateModification = Coalesce(record("oa_patient_note_date_modification"), Nothing)
        patientNote.Invalide = Coalesce(record("oa_patient_note_invalide"), False)
        Return patientNote
    End Function
    ...
    Public Function CreationNote(patientNote As PatientNote) As Boolean
        ...
        Dim SQLstring As String = "insert into oasis.oa_patient_note_social" &
    ...
    Public Function ModificationNote(patientNote As PatientNote) As Boolean
        ...
        Dim SQLstring As String = "update oasis.oa_patient_note_social set" &
    ...
    Public Function AnnulationNote(patientNote As PatientNote) As Boolean
        ...
        Dim SQLstring As String = "update oasis.oa_patient_note_social set oa_patient_note_date_modification = @dateModification," &
