...
Public Class FmDRCListe
    ...
    Private Sub FmDRCListe_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub ChargementDRC()
        ...
        For i = 0 To rowCount Step 1
            ...
            RadDRCDataGridView.Rows.Add(iGrid)

            '------------------- Alimentation du DataGridView
            'Id DRC
            RadDRCDataGridView.Rows(iGrid).Cells("drcId").Value = DRCDataTable.Rows(i)("oa_drc_id")

            'DRC Description
            RadDRCDataGridView.Rows(iGrid).Cells("drcDescription").Value = DRCDataTable.Rows(i)("oa_drc_libelle")

            RadDRCDataGridView.Rows(iGrid).Cells("categorieMajeure").Value = categorieMajeureDescription

            RadDRCDataGridView.Rows(iGrid).Cells("drcCategorie").Value = oasisCategorieDescription

            If drcOasis = True Then
                RadDRCDataGridView.Rows(iGrid).Cells("drcOasis").Value = "Oasis"
                RadDRCDataGridView.Rows(iGrid).Cells("drcOasis").Style.ForeColor = Color.Red
            Else
                RadDRCDataGridView.Rows(iGrid).Cells("drcOasis").Value = ""
            End If
        Next
        ...
        If iGrid > -1 Then
            Me.RadDRCDataGridView.Rows(0).IsSelected = True
            Me.RadDRCDataGridView.Rows(0).IsCurrent = True
            Me.RadDRCDataGridView.Rows(0).EnsureVisible()
        End If
    ...
    Private Sub initZones()
        ...
        'Traitements
        RadDRCDataGridView.Rows.Clear()
    End Sub

    Private Sub DRCDataGridView_DoubleClick(sender As Object, e As EventArgs)
        ...
        If aRow <= maxRow And aRow > -1 Then
            Dim drcId As Integer = RadDRCDataGridView.Rows(aRow).Cells("drcId").Value
    ...
    Private Sub DRCDataGridView_CellClick(sender As Object, e As DataGridViewCellEventArgs)
        Dim aRow, maxRow As Integer
        aRow = Me.RadDRCDataGridView.Rows.IndexOf(Me.RadDRCDataGridView.CurrentRow)
        maxRow = RadDRCDataGridView.Rows.Count - 1
        ...
        If aRow <= maxRow And aRow > -1 Then
            Dim drcId As Integer = RadDRCDataGridView.Rows(aRow).Cells("drcId").Value
            Dim drcLibelle As String = RadDRCDataGridView.Rows(aRow).Cells("drcDescription").Value
            selectedDrcId = drcId
